...
function parseEvent(ev) {
  ...
  return {
    ...
    homeLogo: home.team?.logos?.[0]?.href || home.team?.logo || '',  // ✅ تم إضافة logo
    homeScore: home.score ?? '',
    awayTeam: away.team?.displayName || '',
    awayLogo: away.team?.logos?.[0]?.href || away.team?.logo || '',  // ✅ تم إضافة logo
...
async function handleSummary(url) {
  ...
  try {
    ...
    const mapLineup = (roster) => (roster?.roster || []).map(p => ({
