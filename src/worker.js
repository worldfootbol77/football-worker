...
const CONTINENTAL_RULES = {
    ...
    "fra.1": { 1:{color:"#81D6AC",desc:"دوري أبطال أوروبا"}, 2:{color:"#81D6AC",desc:"دوري أبطال أوروبا"}, 3:{color:"#81D6AC",desc:"دوري أبطال أوروبا"}, 4:{color:"#6CABDD",desc:"تصفيات دوري الأبطال"}, 5:{color:"#6CABDD",desc:"الدوري الأوروبي"}, 6:{color:"#B2BFD0",desc:"دوري المؤتمر"}, "-3":{color:"#FF7F84",desc:"ملحق الهبوط"}, "-2":{color:"#FF7F84",desc:"هبوط"}, "-1":{color:"#FF7F84",desc:"هبوط"} },
    "sau.1": { 1:{color:"#81D6AC",desc:"نخبة آسيا"}, 2:{color:"#81D6AC",desc:"نخبة آسيا"}, 3:{color:"#6CABDD",desc:"دوري أبطال آسيا 2"}, "-3":{color:"#FF7F84",desc:"هبوط"}, "-2":{color:"#FF7F84",desc:"هبوط"}, "-1":{color:"#FF7F84",desc:"هبوط"} }
};
...
function getFlag(name = '') {
  ...
  if (n.includes('premier league')) return '🏴\u200d󠁢󠁥󠁮󠁧󠁿';
...
function extractEvents(data, homeTeamName, awayTeamName) {
  const goals = []; const cards = []; const subs  = []; const seen  = new Set();
  ...
  for (const ev of keyEvents) {
    ...
    if (t === 'goal' || t === 'owngoal') {
      goals.push({ minute: fullMin, player: player1, assist: participants[1]?.athlete?.displayName || '', team, type: t === 'owngoal' ? 'ownGoal' : 'goal' });
    }
  ...
  for (const play of (data.plays || [])) {
    ...
    subs.push({
      ...
      playerIn: playerIn || '—',
      playerOut: playerOut || '—',
      jerseyIn: getJersey(playerIn, pIn.athlete?.jersey || ''),
      jerseyOut: getJersey(playerOut, pOut.athlete?.jersey || ''),
      team
    });
  }
...
async function handleSummary(url, env) {
  ...
  const leaguesToTry = league ? [league, 'fifa.world', 'eng.1', 'esp.1', 'ger.1', 'ita.1', 'fra.1'] : ['fifa.world', 'eng.1', 'esp.1', 'ger.1', 'ita.1', 'fra.1'];
  let data = null; let usedLeague = '';
  for (const lg of leaguesToTry) {
    try {
      ...
      if (d.header?.competitions?.[0]?.competitors?.length) { data = d; usedLeague = league || lg; break; }
    } catch (_) {}
  }
  if (!data) return new Response(JSON.stringify({ error: 'لم يتم العثور على المباراة' }), { status: 404, headers: CORS });

  try {
    const hdr = data.header || {}; const comp = hdr.competitions?.[0] || {};
    const homeComp = comp.competitors?.find(c => c.homeAway === 'home') || {};
    const awayComp = comp.competitors?.find(c => c.homeAway === 'away') || {};
    const homeTeamName = homeComp.team?.displayName || ''; const awayTeamName = awayComp.team?.displayName || '';
    const gi = data.gameInfo?.venue || {}; const addr = gi.address || {};
    const venue = [gi.fullName, addr.city, addr.country].filter(Boolean).join('، ');
    const altNote = comp.altGameNote || ''; const altParts = altNote.split(',').map(s => s.trim());
    const leagueNameOnly = altParts[0] || hdr.league?.name || usedLeague || '';
    const leagueName = `${getFlag(leagueNameOnly)} ${leagueNameOnly}${altParts[1] ? ' - ' + altParts.slice(1).join(', ') : ''}`;

    const { goals, cards, subs, homeSubs, awaySubs } = extractEvents(data, homeTeamName, awayTeamName);
    const homeRoster = data.rosters?.find(r => r.homeAway === 'home'); const awayRoster = data.rosters?.find(r => r.homeAway === 'away');
    const mapLineup = (r) => (r?.roster || []).map(p => ({ name: p.athlete?.displayName || '', shortName: p.athlete?.shortName || '', jersey: p.jersey || '', position: p.position?.abbreviation || '', starter: p.starter ?? false }));

    const result = {
      success: true, id: matchId, league: usedLeague, leagueName, venue, date: comp.date,
      homeTeam: homeTeamName, homeLogo: homeComp.team?.logo || '', homeScore: homeComp.score || '0', homeShootout: homeComp.shootoutScore ?? null,
      awayTeam: awayTeamName, awayLogo: awayComp.team?.logo || '', awayScore: awayComp.score || '0', awayShootout: awayComp.shootoutScore ?? null,
      status: comp.status?.type?.state || 'post', statusText: comp.status?.type?.shortDetail || '', minute: comp.status?.displayClock || '',
      homeFormation: homeRoster?.formation || '', awayFormation: awayRoster?.formation || '',
      goals, cards, subs, homeSubs, awaySubs, homeLineup: mapLineup(homeRoster), awayLineup: mapLineup(awayRoster),
    ...
    await kvPut(env, kvKey, result, result.status === 'in' ? TTL_SUMMARY : TTL_FINISHED);
    return new Response(JSON.stringify(result), { headers: { ...CORS, 'Content-Type': 'application/json' } });
  } catch (e) {
...
async function handleStandings(url, env) {
  ...
  let seasonParam = '';
  if (league === 'fifa.world' || league.includes('worldq')) seasonParam = '?season=2026';
  else if (['uefa.euro','conmebol.copa','uefa.nations'].includes(league)) seasonParam = '?season=2024';
  ...
  try {
    ...
    if (!res.ok) return new Response(JSON.stringify({ success: false, noStandings: true, error: `لا يوجد ترتيب متاح للبطولة (${league})` }), { headers: { ...CORS, 'Content-Type': 'application/json' } });
    const data = await res.json(); const children = data.children || [];
    if (!children.length) return new Response(JSON.stringify({ success: false, noStandings: true, error: 'لا توجد بيانات ترتيب حالياً' }), { headers: { ...CORS, 'Content-Type': 'application/json' } });
    ...
    for (const child of children) {
      ...
      const teams = entries.map(e => {
        const stats = {}; for (const s of (e.stats || [])) stats[s.name] = s.displayValue ?? s.value ?? 0;
        const rank = parseInt(stats.rank) || (entries.indexOf(e) + 1); const fb = applyFallbackColors(league, rank, entries.length);
        let cls = '';
        if (fb.color === '#81D6AC') cls = 'promo'; else if (fb.color === '#6CABDD') cls = 'ucl';
        else if (fb.color === '#B2BFD0') cls = 'playoff'; else if (fb.color === '#FF7F84') cls = 'rel';
        return {
          rank, name: e.team?.displayName || '', logo: e.team?.logos?.[0]?.href || '',
          played: stats.gamesPlayed ?? '', wins: stats.wins ?? '', draws: stats.ties ?? stats.draws ?? '', losses: stats.losses ?? '', gd: stats.pointDifferential ?? '', points: stats.points ?? '',
          note_color: fb.color || '', note_description: fb.desc || '', color_class: cls
        };
      });
    ...
    return new Response(JSON.stringify({ success: false, noStandings: true, error: 'تعذر تحميل الترتيب' }), { headers: { ...CORS, 'Content-Type': 'application/json' } });
  }
}
...
async function handleScorers(url, env) {
  ...
  try {
    ...
    const data = await res.json();
    const goalsCat = (data.categories || []).find(c => (c.name || '').toLowerCase().includes('goal')) || data.categories?.[0];
    const scorers = (goalsCat?.leaders || []).map((l, i) => ({ rank: i + 1, name: l.athlete?.displayName || l.displayName || '', photo: l.athlete?.headshot?.href || '', team: l.team?.displayName || '', teamLogo: l.team?.logos?.[0]?.href || '', goals: parseInt(l.value) || 0 }));
