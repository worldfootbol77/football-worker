...
async function fetchAndStoreSummary(matchId, league, env) {
  ...
  try {
    ...
    // TTL حسب خطتك: مباشر = 17 دقيقة (1020 ثانية)، منتهي = 90 يوم (7776000 ثانية)
...
async function smartRefresh(env) {
  ...
  // إذا لم تكن هناك نوافذ مباريات، تحقق أولاً
...
const NO_STANDINGS = new Set([
  'uefa.europa', 'uefa.europa.conf', 'fifa.world'
]);
