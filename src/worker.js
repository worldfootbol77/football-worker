...
// ─── دالة تحسين اسم الدوري (الأهم) ────────────────────────────────────────────
function getProperLeagueName(leagueCode, originalName, ev) {
  // قائمة الأسماء الصحيحة للدوريات حسب الكود
  const leagueNamesMap = {
    // وديات
    '2026-international-friendly': '🤝 مباراة ودية دولية',
    '2026-womens-international-friendly': '🤝 مباراة ودية دولية - سيدات',
    
    // أيرلندا
    '2026-league-of-ireland-premier': '🇮🇪 الدوري الأيرلندي الممتاز',
    '2026-league-of-ireland-first-division': '🇮🇪 دوري الدرجة الأولى الأيرلندي',
    
    // بوليفيا
    '2026-bolivian-liga-profesional': '🇧🇴 دوري الدرجة الأولى البوليفي',
    
    // البرازيل
    '2026-brasileiro-serie-a': '🇧🇷 الدوري البرازيلي الدرجة الأولى',
    '2026-brasileiro-serie-b': '🇧🇷 الدوري البرازيلي الدرجة الثانية',
    '2026-brasileiro-serie-c': '🇧🇷 الدوري البرازيلي الدرجة الثالثة',
    
    // الأرجنتين
    '2026-argentina-primera-division': '🇦🇷 الدوري الأرجنتيني',
    '2026-argentina-primera-nacional': '🇦🇷 دوري الدرجة الأولى الأرجنتيني',
    '2026-argentina-primera-b': '🇦🇷 دوري الدرجة الثانية الأرجنتيني',
    '2026-argentina-primera-c': '🇦🇷 دوري الدرجة الثالثة الأرجنتيني',
    
    // هولندا
    '2026-eredivisie': '🇳🇱 الدوري الهولندي',
    '2026-eerste-divisie': '🇳🇱 دوري الدرجة الأولى الهولندي',
    
    // البرتغال
    '2026-primeira-liga': '🇵🇹 الدوري البرتغالي',
    
    // إنجلترا
    '2026-premier-league': '🏴󠁧󠁢󠁥󠁮󠁧󠁿 الدوري الإنجليزي الممتاز',
    '2026-efl-championship': '🏴󠁧󠁢󠁥󠁮󠁧󠁿 دوري البطولة الإنجليزية',
    
    // إسبانيا
    '2026-laliga': '🇪🇸 الدوري الإسباني',
    '2026-laliga2': '🇪🇸 دوري الدرجة الثانية الإسباني',
    
    // ألمانيا
    '2026-bundesliga': '🇩🇪 الدوري الألماني',
    '2026-2-bundesliga': '🇩🇪 دوري الدرجة الثانية الألماني',
    
    // إيطاليا
    '2026-serie-a': '🇮🇹 الدوري الإيطالي',
    '2026-serie-b': '🇮🇹 دوري الدرجة الثانية الإيطالي',
    
    // فرنسا
    '2026-ligue-1': '🇫🇷 الدوري الفرنسي',
    '2026-ligue-2': '🇫🇷 دوري الدرجة الثانية الفرنسي',
    
    // أمريكا
    '2026-mls': '🇺🇸 الدوري الأمريكي MLS',
    '2026-usl-championship': '🇺🇸 دوري USL',
    
    // أستراليا
    '2026-a-league': '🇦🇺 الدوري الأسترالي',
    
    // المكسيك
    '2026-liga-mx': '🇲🇽 الدوري المكسيكي',
    
    // كولومبيا
    '2026-colombian-primera-a': '🇨🇴 الدوري الكولومبي',
    
    // تشيلي
    '2026-chilean-primera-division': '🇨🇱 الدوري التشيلي',
    
    // السعودية
    '2026-saudi-pro-league': '🇸🇦 الدوري السعودي',
    
    // قطر
    '2026-qatar-stars-league': '🇶🇦 الدوري القطري',
    
    // الإمارات
    '2026-uae-pro-league': '🇦🇪 دوري الخليج العربي',
    
    // مصر
    '2026-egyptian-premier-league': '🇪🇬 الدوري المصري',
    
    // المغرب
    '2026-botola-pro': '🇲🇦 الدوري المغربي',
    
    // تونس
    '2026-tunisian-ligue-1': '🇹🇳 الرابطة التونسية',
    
    // اليابان
    '2026-j1-league': '🇯🇵 الدوري الياباني',
    
    // كوريا
    '2026-k-league-1': '🇰🇷 الدوري الكوري',
    
    // الصين
    '2026-csl': '🇨🇳 الدوري الصيني',
  ...
  // 1. البحث في الخريطة باستخدام الكود كاملاً
  if (leagueNamesMap[leagueCode]) {
    return leagueNamesMap[leagueCode];
  }
  
  // 2. محاولة استخراج اسم الدوري من season أو league
  if (ev.season?.displayName && !ev.season.displayName.includes(' at ')) {
    return ev.season.displayName;
  }
  if (ev.league?.displayName && !ev.league.displayName.includes(' at ')) {
    return ev.league.displayName;
  }
  if (ev.competitions?.[0]?.league?.displayName) {
    return ev.competitions[0].league.displayName;
  }
  
  // 3. إذا كان الاسم يحتوي على "at"، فهو اسم فريقين وليس دوري
  if (originalName && originalName.includes(' at ')) {
    const codeWithoutYear = leagueCode.replace(/^\d+-/, '');
    const readableName = codeWithoutYear.replace(/-/g, ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
    if (readableName && readableName !== 'Group Stage') {
      return `🏆 ${readableName}`;
    }
    return '🏆 بطولة دولية';
  }
  
  // 4. إزالة "at" من الاسم الأصلي إذا كان بسيطاً
  if (originalName && originalName.includes(' at ')) {
    const parts = originalName.split(' at ');
...
// ─── Parse ESPN Event (معدل لاستخدام تحسين اسم الدوري) ─────────────────────────
function parseEvent(ev) {
  ...
  const originalLeagueName = ev.season?.displayName || ev.league?.displayName || ev.name?.split(':')[0]?.trim() || '';
  const leagueName = getProperLeagueName(leagueCode, originalLeagueName, ev);
