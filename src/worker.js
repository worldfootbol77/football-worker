// ═══════════════════════════════════════════════════════════════════════════════
// src/worker.js — يبحث في جميع الأماكن عن الأحداث
// ═══════════════════════════════════════════════════════════════════════════════
...
// ─── استخراج التبديلات من التشكيلات ──────────────────────────────────────────
function extractSubsFromLineup(lineup, teamName) {
  const subs = [];
  const playersIn = lineup.filter(p => p.subbedIn);
  const playersOut = lineup.filter(p => p.subbedOut);
  
  for (let i = 0; i < Math.max(playersIn.length, playersOut.length); i++) {
...
// ─── دالة البحث عن الأحداث في كل مكان ────────────────────────────────────────
function extractEventsFromAllSources(data) {
  const goals = [];
  const cards = [];
  const allEvents = [];
  ...
  for (const play of data.plays || []) {
    ...
    if (type === 'Goal' || type === 'Own Goal') {
      goals.push({ minute: clock, player, team, type: type === 'Own Goal' ? 'OG' : 'G' });
    }
    if (type.includes('Yellow') || type.includes('Red')) {
      cards.push({ minute: clock, player, team, type });
    }
    allEvents.push({ minute: clock, type, player, team });
  }
  
  // 2. من details (في header)
  ...
  for (const d of details) {
    ...
    if (type.includes('Goal') || type === 'Goal' || type === 'Own Goal') {
      // تجنب التكرار
      if (!goals.some(g => g.minute === clock && g.player === player)) {
        goals.push({ minute: clock, player, team, type: type === 'Own Goal' ? 'OG' : 'G' });
      }
    }
    if (type.includes('Yellow') || type.includes('Red')) {
      if (!cards.some(c => c.minute === clock && c.player === player)) {
        cards.push({ minute: clock, player, team, type });
      }
    }
  }
  
  // 3. من events (في بعض الأحيان)
  for (const ev of data.events || []) {
    ...
    if (type.includes('Goal') || type === 'Goal') {
      if (!goals.some(g => g.minute === clock && g.player === player)) {
        goals.push({ minute: clock, player, team, type: 'G' });
      }
    }
    if (type.includes('Yellow') || type.includes('Red')) {
      if (!cards.some(c => c.minute === clock && c.player === player)) {
  ...
  // 4. من period.events (في بعض الأحيان)
  ...
  // 5. من scoringPlays (في بعض الأحيان)
  for (const sp of data.scoringPlays || []) {
    const player = sp.athleteInvolved?.displayName || '';
    const team = sp.team?.displayName || '';
    if (player && !goals.some(g => g.player === player)) {
      goals.push({ minute: '??', player, team, type: 'G' });
    }
  }
...
async function handleSummary(url) {
  ...
  try {
    ...
    const homeLineup = mapLineup(homeRoster);
    const awayLineup = mapLineup(awayRoster);

    // ─── استخراج الأحداث من كل المصادر ────────────────────────────────────────
    const { goals, cards } = extractEventsFromAllSources(data);
    
    // ─── التبديلات ─────────────────────────────────────────────────────────────
    const homeSubs = extractSubsFromLineup(homeLineup, home.team?.displayName || '');
    const awaySubs = extractSubsFromLineup(awayLineup, away.team?.displayName || '');
