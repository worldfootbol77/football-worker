...
// خريطة لحفظ أسماء البطولات (كاش)
const leagueNameCache = new Map();

// ─── دالة جلب اسم البطولة من summary ─────────────────────────────────────────
async function getRealLeagueName(leagueCode, matchId) {
  ...
  try {
    const url = `${ESPN_LEAGUE}/${leagueCode}/summary?event=${matchId}`;
    ...
    if (res.ok) {
      ...
      // محاولة الحصول على اسم البطولة من عدة أماكن
      let leagueName = data.header?.league?.name || 
                      data.league?.name || 
                      data.competition?.name ||
                      '';
      
      if (leagueName && !leagueName.includes(' at ') && leagueName.length > 2) {
        // تنظيف الاسم
        leagueName = leagueName.replace(/2026/g, '').trim();
        leagueNameCache.set(cacheKey, leagueName);
        return leagueName;
      }
    }
  } catch (e) {
    console.log(`فشل جلب اسم البطولة لـ ${leagueCode}:`, e.message);
  }
  
  return null;
}

// ─── دالة تحسين المباريات ─────────────────────────────────────────────────────
async function enhanceMatches(matches) {
  if (!matches || matches.length === 0) return matches;
  
  // جمع الدوريات الفريدة
  const uniqueLeagues = new Map();
  for (const m of matches) {
    const league = m.league;
    if (league && !uniqueLeagues.has(league)) {
      uniqueLeagues.set(league, m.id);
    }
  }
  
  // جلب أسماء البطولات لكل دوري
  const promises = [];
  for (const [league, matchId] of uniqueLeagues) {
    promises.push(getRealLeagueName(league, matchId));
  }
  await Promise.all(promises);
  
  // تحديث المباريات
  for (const m of matches) {
    const realName = leagueNameCache.get(m.league);
    if (realName) {
      m.leagueName = realName;
    }
  }
...
async function handleMatches(url, env) {
  ...
  try {
    ...
    // 2. تحسين أسماء البطولات (جلب من summary)
...
async function handleSummary(url, env) {
  ...
  try {
    ...
    let leagueName = hdr.league?.name || '';
