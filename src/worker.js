...
function parseEvent(ev) {
  ...
  return {
    id: ev.id, leagueId, league: leagueId, leagueName, leagueNameOnly, leagueFlag, leagueStage,
...
function extractEvents(data, homeTeamName, awayTeamName) {
  ...
  for (const play of (data.plays || [])) {
    ...
    const playerOut = pOut.athlete?.displayName || '';
  ...
  for (const lg of leaguesToTry) {
    try {
      ...
      if (d.header?.competitions?.[0]?.competitors?.length) {
        data = d;
        // استخرج كود الدوري الحقيقي من ESPN
        const espnId = String(d.header?.league?.id || '');
        if (espnId && ID_TO_CODE[espnId]) usedLeague = ID_TO_CODE[espnId];
        else if (league) usedLeague = league;
        else usedLeague = lg;
        break;
      }
    } catch (_) { continue; }
