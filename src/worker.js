...
async function handleSummary(url) {
  ...
  // جرّب عدة مسارات للدوري
  const leaguesToTry = league
    ? [league, 'fifa.world', 'eng.1', 'esp.1', 'ger.1', 'ita.1', 'fra.1', 'bra.1', 'arg.1']
  ...
  try {
    ...
    details.forEach(det => {
      ...
      const minute  = min ? `${min}${addMin}` : '';
      const player  = det.participants?.[0]?.athlete?.displayName || '';
      const assist  = det.participants?.[1]?.athlete?.displayName || '';
      const team    = det.team?.displayName || '';
      ...
      if (det.scoringPlay) {
        ...
        cards.push({ minute, player, team, type: 'redCard' });
      }
    });

    // ── البطاقات الصفراء من الـ roster ────────────────────────────────────────
    ;(data.rosters || []).forEach(r => {
      const teamName = r.team?.displayName || '';
      ;(r.roster || []).forEach(p => {
        const yc = p.yellowCards || 0;
        const rc = p.redCards    || 0;
        if (yc > 0) {
          cards.push({
            minute: '',
        ...
        if (rc > 0 && !cards.some(c => c.player === p.athlete?.displayName && c.type === 'redCard')) {
          cards.push({
            minute: '',
            player: p.athlete?.displayName || '',
            team:   teamName,
    ...
    // ترتيب البطاقات بالدقيقة
    cards.sort((a, b) => (parseInt(a.minute) || 99) - (parseInt(b.minute) || 99));

    // ── التشكيلات والتبديلات ──────────────────────────────────────────────────
    ...
    // بناء قائمة التبديلات من roster
    const buildSubs = (rosterObj) => {
      const out = (rosterObj?.roster || []).filter(p => p.subbedOut);
      const inn = (rosterObj?.roster || []).filter(p => p.subbedIn);
      const max = Math.max(out.length, inn.length);
      const result = [];
      for (let i = 0; i < max; i++) {
        result.push({
          playerIn:   inn[i]?.athlete?.displayName || '',
          jerseyIn:   inn[i]?.jersey || '',
          playerOut:  out[i]?.athlete?.displayName || '',
          jerseyOut:  out[i]?.jersey || '',
          team: rosterObj?.team?.displayName || '',
        });
      }
      return result;
    };
