...
const LEAGUE_ID_MAP = {
  ...
  '3904':  { name: 'بطولة أرجنتينية',               flag: '🇦🇷', slug: 'arg.2'               },
  '4003':  { name: 'دوري أرجنتيني احترافي',          flag: '🇦🇷', slug: 'arg.plen'            },
  '3910':  { name: 'كأس أمم أفريقيا تحت 23',        flag: '🌍', slug: 'caf.u23'              },
  '3916':  { name: 'كأس العالم للشباب تحت 20',      flag: '🌍', slug: 'fifa.world.u20'       },
};
...
function extractLeagueInfo(event) {
  ...
  // بطولة مجهولة — لكن مفتاح فريد (leagueId الحقيقي، ليس 'unknown')
  return {
    leagueId:    leagueId || `other_${stageSlug}_${year}`,
    leagueSlug:  '',
    leagueName:  stage ? `بطولة ${year} · ${stage}` : `بطولة ${year || 'أخرى'}`,
    leagueFlag:  '⚽',
    leagueStage: stage,
  };
}
...
async function fetchSummary(matchId, leagueSlug) {
  ...
  try {
    const res = await fetch(`${ESPN_LEAGUE}/${leagueSlug}/summary?event=${matchId}`, {
...
export default {
  async fetch(request, env) {
    ...
    if (path === '/api/summary') {
      ...
      if (!summary) return new Response(JSON.stringify({ error: 'not found' }),
