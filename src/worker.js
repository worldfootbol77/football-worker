// ═══════════════════════════════════════════════════════════════════════════════
// src/worker.js — النسخة النهائية مع جميع البيانات (من plays + details)
// ═══════════════════════════════════════════════════════════════════════════════
...
// ─── دالة استخراج الأحداث من plays ────────────────────────────────────────────
function extractEventsFromPlays(plays) {
  const goals = [];
  const cards = [];
  const subs = [];
  ...
  for (const play of plays) {
    ...
    const participantIn = play.participants?.[1]?.athlete?.displayName || '';
    ...
    if (type === 'Substitution' || type.includes('Substitution')) {
      subs.push({
        minute: clock,
        playerIn: participant,
        playerOut: participantIn,
        team: team,
      });
    }
  }
...
async function handleSummary(url) {
  ...
  try {
    ...
    const { goals, cards, subs } = extractEventsFromPlays(plays);

    // ─── الإحصائيات ───────────────────────────────────────────────────────────
    const homeStats = data.boxscore?.teams?.[0]?.statistics || [];
    const awayStats = data.boxscore?.teams?.[1]?.statistics || [];

    // ─── استخراج البطاقات من التشكيلات أيضاً ──────────────────────────────────
    const rosterCards = [];
    for (const p of data.rosters || []) {
    ...
    // دمج البطاقات من المصدرين
    const allCards = [...cards, ...rosterCards];
