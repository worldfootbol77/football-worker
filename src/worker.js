// ═══════════════════════════════════════════════════════════════════════════════
// src/worker.js — النسخة النهائية المُصلَحة والموحدة بالكامل لـ Scorio
// ═══════════════════════════════════════════════════════════════════════════════
...
// ─── خريطة تحويل معرّفات ESPN الرقمية إلى رموز الدوريات النصية ───────────────────
const idToCode = {
  "2": "uefa.champions",
  "3": "uefa.europa",
  "9": "fra.1",
...
async function handleSummary(url, env) {
  ...
  if (league && !isNaN(league) && idToCode[league]) {
    league = idToCode[league];
  }
...
async function handleStandings(url, env) {
  ...
  if (!isNaN(league) && idToCode[league]) {
    league = idToCode[league];
  } else if (league) {
    const lgLower = league.toLowerCase();
    if (lgLower.includes('chinese') || lgLower.includes('china')) league = 'chn.1';
    else if (lgLower.includes('saudi') || lgLower.includes('ksa')) league = 'ksa.1';
    else if (lgLower.includes('japan') || lgLower.includes('j1')) league = 'jpn.1';
    else if (lgLower.includes('korea') || lgLower.includes('k1')) league = 'kor.1';
    else if (lgLower.includes('qatar')) league = 'qat.1';
    else if (lgLower.includes('iraq')) league = 'irq.1';
    else if (lgLower.includes('jordan')) league = 'jor.1';
    else if (lgLower.includes('algeria')) league = 'alg.1';
    else if (lgLower.includes('tunisia')) league = 'tun.1';
    else if (lgLower.includes('egypt')) league = 'egy.1';
    else if (lgLower.includes('morocco')) league = 'mar.1';
    else if (lgLower.includes('uae')) league = 'uae.1';
    else if (lgLower.includes('champions league') || lgLower.includes('ucl')) league = 'uefa.champions';
    else if (lgLower.includes('europa')) league = 'uefa.europa';
    else if (lgLower.includes('premier') || lgLower.includes('england')) league = 'eng.1';
    else if (lgLower.includes('la liga') || lgLower.includes('spain')) league = 'esp.1';
    else if (lgLower.includes('serie a') || lgLower.includes('italy')) league = 'ita.1';
    else if (lgLower.includes('bundesliga') || lgLower.includes('germany')) league = 'ger.1';
    else if (lgLower.includes('ligue 1') || lgLower.includes('france')) league = 'fra.1';
  }
...
async function handleScorers(url, env) {
  ...
  try {
    ...
    const res = await fetch(
      `https://site.api.espn.com/apis/site/v2/sports/soccer/${league}/leaders${seasonParam}`,
      { headers: { 'User-Agent': 'Mozilla/5.0' } }
    );
    if (!res.ok) throw new Error(`ESPN ${res.status}`);
    const data = await res.json();
