...
function extractEvents(data, homeTeamName, awayTeamName) {
  ...
  for (const play of plays) {
    ...
    const playerOut = participants.find(p => p.type === 'playerSubstituted')?.athlete?.displayName
                   || participants[0]?.athlete?.displayName || '';
    const playerIn  = participants.find(p => p.type === 'playerSubstituting')?.athlete?.displayName
                   || participants[1]?.athlete?.displayName || '';
    const jerseyOut = participants.find(p => p.type === 'playerSubstituted')?.jersey
                   || participants[0]?.jersey || '';
    const jerseyIn  = participants.find(p => p.type === 'playerSubstituting')?.jersey
                   || participants[1]?.jersey || '';
  ...
  if (subs.length === 0) {
    for (const ev of keyEvents) {
      ...
      subs.push({
        minute:    fullMin,
        playerIn:  isOut ? player2 : player1,
        playerOut: isOut ? player1 : player2,
        jerseyIn:  isOut ? (participants[1]?.jersey || '') : (participants[0]?.jersey || ''),
        jerseyOut: isOut ? (participants[0]?.jersey || '') : (participants[1]?.jersey || ''),
        team,
      });
    }
  }
