// ═══════════════════════════════════════════════════════════════════════════════
// src/worker.js — النسخة النهائية مع دعم leagueId الرقمي
// ═══════════════════════════════════════════════════════════════════════════════
...
async function handleSummary(url) {
  ...
  // محاولة الحصول على leagueId الرقمي من /api/matches
  let leagueId = league;
  try {
    const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
    const matchesRes = await fetch(`${ESPN_ALL}?dates=${date}&limit=500`, {
      headers: { 'User-Agent': 'Mozilla/5.0' }
    });
    const matchesData = await matchesRes.json();
    for (const ev of matchesData.events || []) {
      if (ev.id === matchId) {
        const uid = ev.uid || '';
        const id = uid.match(/~l:(\d+)~/)?.[1] || '';
        if (id) {
          leagueId = id;
        }
        break;
  ...
  // بناء قائمة الرموز للمحاولة
  const leaguesToTry = [];
  
  // 1. الرقمي إذا وجد
  if (leagueId && /^\d+$/.test(leagueId)) {
    leaguesToTry.push(leagueId);
  }
  
  // 2. جميع slugs المعروفة
  const allSlugs = [
    'fifa.world', 'eng.1', 'eng.2', 'eng.3', 'eng.4',
    'esp.1', 'esp.2', 'ger.1', 'ger.2', 'ita.1', 'ita.2',
    'fra.1', 'fra.2', 'bra.1', 'bra.2', 'bra.3', 'bra.c',
    'arg.1', 'arg.2', 'ned.1', 'por.1', 'sco.1', 'bel.1',
    'tur.1', 'gre.1', 'rus.1', 'sau.1', 'qat.1', 'uae.1',
    'egy.1', 'mar.1', 'tun.1', 'usa.1', 'mex.1', 'chi.1',
    'col.1', 'per.1', 'ecu.1', 'uru.1', 'ven.1', 'bol.1',
    'par.1', 'jpn.1', 'kor.1', 'chn.1', 'aus.1', 'ind.1',
    'uefa.champions', 'uefa.europa', 'conmebol.libertadores', 'conmebol.sudamericana'
  ];
  
  for (const slug of allSlugs) {
    if (!leaguesToTry.includes(slug)) {
      leaguesToTry.push(slug);
    }
  }
  
  // 3. league الأصلي
  if (league && !leaguesToTry.includes(league)) {
    leaguesToTry.push(league);
  }
