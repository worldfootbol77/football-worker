...
// ─── Parse ESPN Event (المعدل - يستخدم leagueId كمفتاح فريد) ───────────────────
function parseEvent(ev) {
  ...
  // ✅ استخراج leagueId من uid (الحل النهائي)
  const uid = ev.uid || '';
  const leagueId = uid.match(/l:(\d+)/)?.[1] || '';
  const seasonSlug = ev.season?.slug || '';
  
  // ✅ استخدم leagueId كمفتاح فريد (بدلاً من seasonSlug)
  const leagueCode = leagueId || (slugToLeagueCode(seasonSlug) || seasonSlug);
  ...
  return {
    id: ev.id,
    league: leagueCode,           // الآن "606" لكأس العالم، "775" لدوري الأبطال
    leagueId: leagueId,           // أضف هذا الحقل للاستخدام المستقبلي
    leagueName: leagueName,       // "🌍 كأس العالم 2026 - دور المجموعات"
